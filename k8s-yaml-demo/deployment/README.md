...
> 参考：https://blog.51cto.com/breaklinux/2610757
...
kind: Deployment
metadata:
  name: ops-nginx-api  # 该Deploy的名称，应与应用名称保持一致
  namespace: default   # 指定 命名空间，应根据服务类型划分，比如：ops,test,prod,
  labels:  # 该 Deploy 的标签，主要用于标识该 Deploy 的属性
    appName: ops-nginx-api  # appName 标识该服务
spec:
  selector:
    matchLabels:
      app: ops-nginx-api  # app标签名称，关联该app
  replicas: 5 # Pod的副本数
      ...
      containers:
        - name: ops-nginx-api # 容器名称，与应用名称保持一致
          ...
          livenessProbe:  #存活探针器配置
            failureThreshold: 3 
            # 处于成功时状态时，探测操作至少连续多少次的失败才被视为检测不通过，显示为#failure属性.默认值为3,最小值为 1,存活探测情况下的放弃就意味着重新启动容器。
